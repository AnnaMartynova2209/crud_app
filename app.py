...
# df = pd.read_csv(r'store\data_1.csv')
# # print(df)
# df.to_sql('auto_tmp', con = conn, if_exists = 'replace')
...
# csv_to_sql(r'store\data.csv','auto_tmp')
# cursor.execute('select * from auto_tmp')
# for row in cursor.fetchall():
# 	print(row)
...
def init_auto_hist():
	...
	cursor.execute('''
		CREATE VIEW if not exists v_auto as 
			...
			price,milage
			from auto_hist
			where deleted_flg = 0 and current_timestamp between start_dttm and end_dttm
	''')
...
def create_changed_rows():
	cursor.execute('''
		CREATE TABLE changed_rows_tmp as 
			...
			ON t1.auto_key = t2.auto_key
			and ( t1.model <> t2.model OR t1.transmission <> t2.transmission 
				OR t1.body_type <> t2.body_type OR t1.drive_type <> t2.drive_type OR t1.color <> t2.color 
				OR t1.production_year <> t2.production_year OR t1.engine_capacity <> t2.engine_capacity 
				OR t1.horsepower <> t2.horsepower OR t1.engine_type <> t2.engine_type  OR t1.price <> t2.price 
...
def update_auto_hist():
	cursor.execute('''
		INSERT INTO auto_hist (
			model,transmission,body_type,drive_type,color,production_year,auto_key,engine_capacity,horsepower,engine_type,price,milage
		) SELECT
			model,transmission,body_type,drive_type,color,production_year,auto_key,engine_capacity,horsepower,engine_type,price,milage
		FROM new_rows_tmp
	''')
	...
	cursor.execute('''
		INSERT INTO auto_hist(
			model,transmission,body_type,drive_type,color,production_year,auto_key,engine_capacity,horsepower,engine_type,price,milage,deleted_flg
		) SELECT
			model,transmission,body_type,drive_type,color,production_year,auto_key,engine_capacity,horsepower,engine_type,price,milage,1
...
# создать представление v_auto (актуальный срез данных), заменить auto_hist на v_auto при формировании временных таблиц
